...
class _LoaderState extends State<Loader>  with SingleTickerProviderStateMixin{
  ...
  void initState(){
    ...
    _animationRotation = new Tween(
      begin: 0.0,
      end: 1.0
    ).animate(CurvedAnimation(parent: _animationController, curve: Interval(0.0, 1.0, curve: Curves.linear)));
    ...
    return Container(
      ...
      child: Center(
        child: RotationTransition(
          ...
          child: Stack(
            children: <Widget>[
              Dot(radius: 10, color: Colors.blueGrey[400]),
              Transform.translate(
                offset: Offset(cos(pi / _radianStep) * _radius, sin(pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(2 * pi / _radianStep) * _radius, sin(2 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(3 * pi / _radianStep) * _radius, sin(3 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(4 * pi / _radianStep) * _radius, sin(4 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius , color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(5 * pi / _radianStep) * _radius, sin(5 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(6 * pi / _radianStep) * _radius, sin(6 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(7 * pi / _radianStep) * _radius, sin(7 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              ),
              Transform.translate(
                offset: Offset(cos(8 * pi / _radianStep) * _radius, sin(8 * pi / _radianStep) * _radius),
                child: Dot(radius: _dotRadius, color: Colors.blueGrey[600]),
              )
            ],
          ),
